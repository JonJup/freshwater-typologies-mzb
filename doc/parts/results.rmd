...
Regional typologies performed best in five out of six quality criteria and the single exception ($ASW_max$) is by a narrow margin (Figure \@ref(fig:fig-res-sw)).  
Further, both regional typologies received higher scores than all reach-based typologies three of the six criteria. 
Between the two, Illies freshwater ecoregions accrued the higher overall score due to the low scores of the BGR in CS and IVS.
The two types of BRT typologies received similar scores for most quality criteria. For all criteria except CS, BRT12 scored slightly higher than BRT20.   
<!-- ASW -->
...
Those of reach-based typologies were slightly lower. 
With $p = 1$, only the ASW of BGR exceeded zero and hence the mean of random typologies. 
The difference between region- and reach-based typologies narrowed.
The $ASW_{max}$ was highest for BRT12 but negative for all typologies, including the biological. 


```{r fig-res-sw, fig.align="center", fig.height=5, fig.cap="Cluster validity citeria for freshwater typologies. The points represent the typologies which are indicated by their color. The biological typology and the mean of random typologies are represented by lines. Each plot shows a different cluster validity criterion: Calinski Harabasz (CH), Classification Strength (CS), Indicator Value score (IVS), average minimal silhouette width (min), average silhouette width (average) and average maximal silhouette width (max). For all criteria high scores imply better classifications."}
...
The mean number of taxa in typical assemblages was similar in all typologies (`r round(mean(out_list$value),1)`). 
...
The differences are more pronounced between individual typical assemblages. 
...
```{r fig-redundancy, fig.cap="Similarity between typical assemblages for five freshwater typologies. The colored dots are similarities between two types of the same typology. The typologies differ in the number of types therefore the number of comparisons differs between typologies. The Jaccard distance we used is asymmetric which increases the number of distinguisahble comparisons. The violins represent their frequency distributions. The black dots are the arithmetic means of similarities."}
redundancy_plot
```

The fraction of all 1114 taxa that was included in the typical assemblage varied considerably between typologies. 
The typical assemblages of BRT12 included `r ta.perc.brt12` of all taxa, those of BRT20 `r ta.perc.brt20`, those of GLoRiC `r ta.perc.gloric`, those of Illies `r ta.perc.illies` and those of BGR `r ta.perc.bgr`. 
On average, a taxon occurred in `r ta.mu.occ$rt1` typical assemblages in BRT12, `r ta.mu.occ$rt2` in BRT20, `r ta.mu.occ$glo` in GloRiC, `r ta.mu.occ$ill` in Illies and `r ta.mu.occ$bgr` in BGR.
The two taxa that occurred most frequently in typical assemblages were both families: Chironomidae (49 of 56 typical assemblages) and Simuliidae (40). 
...
However, the relation of these types to environmental features was not specified and therefore eludes from interpretation.
For Illies, we merged the type England with the type Ireland and Northern Ireland as well as Western plains with Western highlands.
