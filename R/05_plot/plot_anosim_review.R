...
#       Purpose: Plot ANOSIM results for genus-level relative abundance data
# -----------------------------------
...
sink(file = paste0("R/05_/log_files/anosim_review","_", Sys.Date(), "_", "log.txt"))
Sys.Date()
x$path
sessionInfo()
sink(file = NULL)
...
# load data -------------------------------------------------------------------------
data <- readRDS("data/review/04_2022-04-29_results_anosim_review.rds")
data$season <- c("autumn", "autumn", "spring", "spring", "summer", "summer", "autumn", "spring", "summer")
...
shapes = c(21,22,23)
anosim_plot <- ggplot(data2, aes(x = R, y = typology)) + 
...
# - save anosim plot as rds to combine it with ANOSIM plot in another scripts
saveRDS(anosim_plot, paste0("fig/results_anosim/", Sys.Date(),"_anosim_plot.rds"))
