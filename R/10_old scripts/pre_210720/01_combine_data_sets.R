...
#- IDS of rivpacs summer samples 
set09 |> 
        filter(season == "summer") |> 
        pull(gr_sample_id) |> 
        unique() -> 
        summer_ids 
saveRDS(summer_ids, "data/summer season/rivpacs_summer_ids.rds")

set01 |> 
        unique(by = "site_id") |> 
        st_write("landau.gpkg")


test <- set04[gr_sample_id == "site_02574_date_01577_mzb_Naiades"]

nrow(test)
setorderv(test, c("genus", "family", "order"))
View(test)
uniqueN(test, "genus")
...
for (i in seq_along(ch_files)){
        if (i == 1){
                tax.res <- data.table (data.set = ch_files, 
                                       tax.res  = 0)
        }
        y = get(ch_files[i])
        x = copy(y)
        setDT(x)
        x[!is.na(species), taxonomic_resolution := 1]
        x[is.na(taxonomic_resolution) & !is.na(genus),  taxonomic_resolution:= 2]
        x[is.na(taxonomic_resolution) & !is.na(family),  taxonomic_resolution:= 3]
        x[is.na(taxonomic_resolution),  taxonomic_resolution:= 4]
        z <- round(mean(x$taxonomic_resolution),2)
        tax.res$tax.res[i] <- z
}
