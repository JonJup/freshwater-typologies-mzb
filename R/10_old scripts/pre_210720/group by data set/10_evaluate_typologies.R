...
# files out:  
#               <- group by data set/auxilliary/10_cluster.eval.obj.rds
#               <- group by data set/auxilliary/10_cluster.eval.obj_random.rds
#               <- group by data set/auxilliary/10_gensil_output.rds
#               <- group by data set/auxilliary/10_gensil_output_random.rds
#               <- group by data set/10_class_eval_group by data set.rds
# Purpose: 
...
# load data  ----------------------------------------------------------------------------------------------------------------------------------------------
data <- readRDS("data/group by data set/09_sxs_genus_typology_with_bio.rds")
...
ntc <- which(names(data) %in% c("gr_sample_id", "data.set", "bio7", "bio12"))
...
saveRDS(cluster.eval.obj  , "data/group by data set/auxilliary/10_cluster.eval.obj.rds")
cluster.eval.obj <- readRDS("data/group by data set/auxilliary/10_cluster.eval.obj.rds")
...
saveRDS(cluster.eval.obj2  , "data/group by data set/auxilliary/10_cluster.eval.obj_random.rds")
cluster.eval.obj2 <- readRDS("data/group by data set/auxilliary/10_cluster.eval.obj_random.rds")
...
saveRDS(gensil_output, "data/group by data set/auxilliary/10_gensil_output.rds")
gensil_output <- readRDS("data/group by data set/auxilliary/10_gensil_output.rds")
...
saveRDS(gensil_output2,   "data/group by data set/auxilliary/10_gensil_output_random.rds")
gensil_output2 <- readRDS("data/group by data set/auxilliary/10_gensil_output_random.rds")
...
# save to file  ---------------------------------------------------------------------
saveRDS(final, "data/group by data set/10_class_eval_data.set.rds")
...
library(cluster)  
sil.obj <- silhouette(x = as.integer(as.factor(data$data.set)), dist = similaitry.matrix)

sil.data <- data.table(cl = sil.obj[,1], 
                       sw = sil.obj[,3])

sil.join <- data.table(data.set = unique(as.factor(data$data.set)),
                       cl = 1:14)

sil.data <- sil.join[sil.data, on = "cl"]

ggplot(sil.data, aes(x = data.set, y = sw)) + geom_point() + geom_violin(draw_quantiles = 0.5)
