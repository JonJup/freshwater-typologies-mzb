...
# load data -------------------------------------------
sxs <- readRDS("data/temporal_aggregation/07_sxs_genus_all_typologies.rds") 
...
saveRDS(similaitry.matrix, "data/temporal_aggregation/auxilliary/09_distance_dice.rds")
...
#- save or load cluster object  
saveRDS(cluster.obj, "data/temporal_aggregation/auxilliary/09_beta_cluster.rds")
cluster.obj     <- readRDS("data/temporal_aggregation/auxilliary/09_beta_cluster.rds")

#- create list with different tree sizes 
...
cluster.eval.obj <- lapply(1:length(cluster.obj.cut),
                           function(x)
                                   cluster.stats(d          = similaitry.matrix,
                                                 clustering = cluster.obj.cut[[x]]));beepr::beep()
#- save to file 
saveRDS(cluster.eval.obj, "data/temporal_aggregation/auxilliary/09_cluster.eval.obj.rds")
cluster.eval.obj <- readRDS("data/temporal_aggregation/auxilliary/09_cluster.eval.obj.rds")
...
sa.eval.obj3 <- data.table(typology = 2:30, 
                           indval   = sa.eval.obj2)
...
saveRDS(gensil_output, "data/temporal_aggregation/auxilliary/09_gensil_output.rds")
...
sxs$bio4 <- cutree(cluster.obj, k = 4) 
...
saveRDS(sxs, "data/temporal_aggregation/09_sxs_genus_typology_with_bio.rds")
