---
title: 'Individual Data Set: STAR'
...
map |>  pull(bgr) |> table()
map |>  pull(illies) |> table()
map |>  pull(brt12) |> table()


c(nrow(da[is.na(species)])/nrow(da))
c(nrow(da[is.na(genus)])  /nrow(da))
c(nrow(da[is.na(family)]) /nrow(da))

#- species missing in ~ 50% of observations 
#- genus missing in ~ 10% of observations 
#- family missing in < 1% of observation
```
...
if (!options$pre){ 
        ...
        das2 <- prep1(data = list(da), 
                      taxon = "species")
        dag2 <- prep1(data = list(da),
                      taxon = "genus")
        daf2 <- prep1(data = list(da),
                      taxon = "family")
        lis2 <- prep1(data = list(li),
                      taxon = "species")
        lig2 <- prep1(data = list(li),
                      taxon = "genus")
        lif2 <- prep1(data = list(li),
                      taxon = "family")
        pus2 <- prep1(data = list(pu),
                      taxon = "species")
        pug2 <- prep1(data = list(pu),
                      taxon = "genus")
        puf2 <- prep1(data = list(pu),
                      taxon = "family")
        lis2.abu <- prep1(data = list(li), taxon = "species", abundance.to.pa = FALSE)
        lig2.abu <- prep1(data = list(li), taxon = "genus", abundance.to.pa = FALSE)
        lif2.abu <- prep1(data = list(li), taxon = "family", abundance.to.pa = FALSE)
...
mantel.matrix <- matrix(0,nrow = 6, ncol = 6)
diag(mantel.matrix) <- 1
tax.lvl <- c("species (abu)", "species (pa)","genus (abu)","genus (pa)", "family (abu)", "family (pa)")
rownames(mantel.matrix) = colnames(mantel.matrix) = tax.lvl
...
#- loop over seasons 
 ...
 for (k in 1:nrow(mantel.matrix)){ for (j in 1:ncol(mantel.matrix)){
     if (k == j) next()
     j.from = rownames(mantel.matrix)[k]; j.to   = colnames(mantel.matrix)[j]
     ...
     mantel.matrix[k,j] <- j.sub
     rm(list = ls()[grepl("^j\\.",ls())])
 }}

corrplot(mantel.matrix, method = "shade", type = "lower", diag = FALSE, addCoef.col = "black", mar=c(0,0,1,0), 
