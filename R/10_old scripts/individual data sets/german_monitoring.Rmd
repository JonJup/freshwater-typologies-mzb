...
data.set = "german_monitoring"
all_seasons <- c("spring", "summer", "autumn", "winter")
seasons <- 1:3
typologies <- c("brt12", "german_type", "illies", "bgr", "brt12_illies", "brt12_bgr")
```
...
if (!options$pre){
  ...
  da <-
    lapply(data, function(x)
      x[data.set == "German federal monitoring data"])
  
  #- check abundance variable 
  #str(data$spring$abundance)
  
  da <- lapply(seasons, function(x)
    da[[x]][!is.na(german_type)])
  ...
  pu <-
    lapply(seasons, function(x)
      da[[x]][illies_distance >= 25000 & bgr_distance >= 25000])
  ...
  sapply(1:3, function(x)
    uniqueN(pu[[x]]$brt12))
  sapply(1:3, function(x)
    uniqueN(pu[[x]]$brt12_illies))
  sapply(1:3, function(x)
    uniqueN(pu[[x]]$brt12_bgr))
  sapply(1:3, function(x)
    uniqueN(pu[[x]]$german_typ))
  das2 <- 
    prep1(data = da,
                taxon = "species",
                seasons = c(1:3))
...
if (!options$mantel) {
  ...
  m2 <-
    lapply(1:3, function(x)
      mantel.rtest(dag.dist[[x]], daf.dist[[x]]))
  m2 <- sapply(m2, function(x)
    x$obs)
  m3 <-
    lapply(1:3, function(x)
      mantel.rtest(dag.abu.dist[[x]], dag.dist[[x]]))
  m3 <- sapply(m3, function(x)
    x$obs)
  m4 <-
    lapply(1:3, function(x)
      mantel.rtest(daf.dist[[x]], daf.abu.dist[[x]]))
  m4 <- sapply(m4, function(x)
    x$obs)
  saveRDS(list(
    m2 = m2, 
    m3 = m3, 
...
The mantel correlation between genus and family level are `r m2`. 
Between abundance and PA in genus the correlation is `r m3` and `r m4` for familiy.
...
saveRDS(object = list(anas,anas,anag,anaf,anls,anlg,anlf,anps,anpg,anpf), 
        "auxilliary/anosim_german_monitoring.rds")
...
saveRDS(object = list(isas,isag,isaf,isls,islg,islf,isps,ispg,ispf), 
        "auxilliary/indi_typ_german_monitoring.rds")
...
fwrite(summary.out, file = "german_monitoring.csv")
```
