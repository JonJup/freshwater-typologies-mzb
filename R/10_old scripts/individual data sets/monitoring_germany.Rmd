---
title: 'Individual Data Set: Monitoring Germany'
...
da <-
        lapply(data, function(x)
                x[data.set == "Monitoring data from Germany"])
...
map <- 
  ...
  st_as_sf()
tm_shape(map) + tm_dots(col = "fec.least.impacted") + tm_facets(by = "season")
```
...
#- two bgr regions with many samples (Atlantic and Continental). Continental always much more samples than Atlantic.
#- Alps always included but aways < 10 sites 
...
#- Always four regions (Alps, Central Highland, Central Plain, Western Highland). In spring also Western plain (n=5). 
#- Only Central highlands and plains are always >10 samples. 
...
#- species missing from ~ 30% of data
#- genus missing from ~ 9% of data 
#- family missing from < 1% of data 

```

```{r GLOBAL VARIBLES 2}
seasons <- 1:3
typologies <- c("brt12", "brt12_illies", "brt12_bgr", "german_type")
```
...
```{r RESHAPE MANTEL TEST}
#- correlation marices for each season 
mantel.spring <- mantel.summer <- mantel.autumn <-  matrix(NA,nrow = 6, ncol = 6)
diag(mantel.spring) <- diag(mantel.summer) <- diag(mantel.autumn) <- 1
 tax.lvl <- c("species (abu)", "species (pa)","genus (abu)","genus (pa)", "family (abu)", "family (pa)")
 rownames(mantel.spring) = colnames(mantel.spring) =
   rownames(mantel.summer) = colnames(mantel.summer) =
   rownames(mantel.autumn) = colnames(mantel.autumn) = tax.lvl
from.vec <- rep(tax.lvl, times = c(3,2,1,0,1,0))

mantel.results <- data.table(
  from = rep(from.vec, each = 3),
  to   = rep(c("species (pa)", "genus (abu)", "genus (pa)", "genus (abu)", "genus (pa)", "genus (pa)", "family (pa)"),
    each = 3),
  statistic = c(
