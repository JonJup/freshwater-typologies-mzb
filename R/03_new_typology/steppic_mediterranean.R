### --- BRT X BGR - steppic - MEDITERRANEAN --- ### 

#- extract steppic Mediterranean data 
data.med <-
        data |>
        filter(short_name == "steppic" & m_btype12 %in% c("RT11", "RT12"))
...
setDT(geo.med)
geo.med[(geology == "unknown" | is.na(geology)) & acid_basic != "sediments", geology := acid_basic]
geo.med %<>% st_as_sf()
...
data[dominant_geology == "calcareous" & m_btype20c == "RT17" & short_name == "steppic" , brt_bgr := "RT02_steppic"]
data[dominant_geology == "siliceous"  & m_btype20c == "RT17" & short_name == "steppic" , brt_bgr := "RT04_steppic"]
data[dominant_geology == "calcareous" & m_btype20c == "RT18" & short_name == "steppic" , brt_bgr := "RT06_steppic"]
data[dominant_geology == "siliceous"  & m_btype20c == "RT18" & short_name == "steppic" , brt_bgr := "RT08_steppic"]
data[dominant_geology == "calcareous" & m_btype20c == "RT19" & short_name == "steppic" , brt_bgr := "RT07_steppic"]
data[dominant_geology == "siliceous"  & m_btype20c == "RT19" & short_name == "steppic" , brt_bgr := "RT09_steppic"]
data[dominant_geology == "calcareous" & m_btype20c == "RT20" & short_name == "steppic" , brt_bgr := "RT07_steppic"]
data[dominant_geology == "siliceous"  & m_btype20c == "RT20" & short_name == "steppic" , brt_bgr := "RT09_steppic"]
data[, dominant_geology := NULL]
data <- st_as_sf(data)

data |> 
        filter(short_name == "steppic") |> 
        pull(brt_bgr) |> 
        unique()

saveRDS(data, "R/new_typology/auxilliary/data_after_steppic.RDS")
