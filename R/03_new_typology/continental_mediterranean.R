### --- BRT X BGR - continental - MEDITERRANEAN --- ### 

#- extract continental Mediterranean data 
data.med <-
        data |>
        filter(short_name == "continental" & m_btype12 %in% c("RT11", "RT12"))
...
for (i in 1:nrow(fec.med)){
        ...
        fec.med[i,"dominant_geology"] <- "missing"
        print("missing")
        next()
}
...
fec.med |> 
        setDT() |> 
        {\(x) x[dominant_geology == "unknown", dominant_geology := "calcareous"]}() |> 
        {\(x) x[dominant_geology == "calacareous", dominant_geology := "calcareous"]}() |> 
        {\(x) x[dominant_geology == "missing", dominant_geology := "calcareous"]}() |> 
        {\(x) x[dominant_geology == "mixed", dominant_geology := "calcareous"]}() |> 
        st_as_sf() -> 
        fec.med 
...
setDT(fec.med)
fec.med[is.na(dominant_geology), dominant_geology := "calcareous"]
fec.med %<>% st_as_sf()
...
data[dominant_geology == "calcareous" & m_btype20c == "RT17" & short_name == "continental" , brt_bgr := "RT02_continental"]
data[dominant_geology == "siliceous"  & m_btype20c == "RT17" & short_name == "continental" , brt_bgr := "RT04_continental"]
data[dominant_geology == "calcareous" & m_btype20c == "RT18" & short_name == "continental" , brt_bgr := "RT06_continental"]
data[dominant_geology == "siliceous"  & m_btype20c == "RT18" & short_name == "continental" , brt_bgr := "RT08_continental"]
data[dominant_geology == "calcareous" & m_btype20c == "RT19" & short_name == "continental" , brt_bgr := "RT07_continental"]
data[dominant_geology == "siliceous"  & m_btype20c == "RT19" & short_name == "continental" , brt_bgr := "RT09_continental"]
data[dominant_geology == "calcareous" & m_btype20c == "RT20" & short_name == "continental" , brt_bgr := "RT07_continental"]
data[dominant_geology == "siliceous"  & m_btype20c == "RT20" & short_name == "continental" , brt_bgr := "RT09_continental"]

data[m_zhyd == "W3X1000001_1", brt_bgr := "RT07_continental"]

data[, dominant_geology := NULL]
data <- st_as_sf(data)

data |> 
        filter(short_name == "continental") |> 
        pull(brt_bgr) |> 
        unique()

saveRDS(data, "R/new_typology/auxilliary/data_after_continental1.rds")
